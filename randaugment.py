...
def Cutout(img, v, max_v, **kwarg):
    ...
    w, h = img.size
    x0 = paddle.FloatTensor(1, ).uniform_(0, w).item()
    y0 = paddle.FloatTensor(1, ).uniform_(0, h).item()
    x0 = int(max(0, x0 - v / 2.))
    y0 = int(max(0, y0 - v / 2.))
    x1 = int(min(w, x0 + v))
    y1 = int(min(h, y0 + v))
...
class RandAugment(object):
    ...
    def __call__(self, img):
        ops = [self.augment_pool[i] for i in paddle.randint(len(self.augment_pool), (self.n,)).tolist()]
        for op, max_v, bias in ops:
            prob = paddle.FloatTensor(1, ).uniform_(0.2, 0.8).item()
            if paddle.rand(1,).item() + prob >= 1:
