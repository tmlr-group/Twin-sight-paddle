...
class PSAggregator(object):
    ...
    def create_data_loaders_from_arrays(self, X_train, y_train, X_test, y_test, batch_size):
        train = data.TensorDataset(
            paddle.to_tensor(X_train), paddle.to_tensor(y_train)
        )
        ...
        test = data.TensorDataset(
            paddle.to_tensor(X_test), paddle.to_tensor(y_test)
        )
    ...
    def save_server_model(self,path):
        paddle.save(self.trainer.model.SimCLR.online_encoder.cpu().state_dict(), path)

    def update_centroids(self, client_centroids, weights):
        for i in range(self.args.num_classes):
            class_i_centroid = paddle.zeros(self.class_centroids[i].shape)
            sum = 0
