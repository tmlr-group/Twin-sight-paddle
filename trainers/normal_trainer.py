...
class NormalTrainer(object):
    ...
    def _set_optimizer(self):
        ...
        self.optimizer = paddle.optim.SGD(params_to_optimizer,
    ...
    def set_SemiFed_BYOL_model_for_gloabl(self,online_encoder, online_predictor, sup_model):
        self.model.unsup_model.online_encoder.set_state_dict(online_encoder)
        self.model.unsup_model.online_predictor.set_state_dict(online_predictor)
        self.model.sup_classifier.set_state_dict(sup_model)
    ...
    def set_model_params(self, params):
        ...
        if self.args.model == 'SemiFed_BYOL':
            ...
            self.model.unsup_model.online_encoder.set_state_dict(params['UNSUP_ONLINE_ENCODER'])
            self.model.unsup_model.online_predictor.set_state_dict(params['UNSUP_ONLINE_PREDICTOR'])
            self.model.sup_classifier.set_state_dict(params['SUP_MODEL'])
        else:
...
# got it Batch_Normalization set
    ...
    def cal_time_p_and_p_model(self,logits_x_ulb_w, time_p, p_model, label_hist):
        prob_w = paddle.softmax(logits_x_ulb_w, dim=1) 
        max_probs, max_idx = paddle.max(prob_w, dim=-1)
        if time_p is None:
            time_p = max_probs.mean()
        else:
            time_p = time_p * 0.999 +  max_probs.mean() * 0.001
        if p_model is None:
            p_model = paddle.mean(prob_w, dim=0)
        else:
            p_model = p_model * 0.999 + paddle.mean(prob_w, dim=0) * 0.001
        if label_hist is None:
            label_hist = paddle.bincount(max_idx, minlength=p_model.shape[0]).to(p_model.dtype) 
            label_hist = label_hist / label_hist.sum()
        else:
            hist = paddle.bincount(max_idx, minlength=p_model.shape[0]).to(p_model.dtype) 
    ...
    def make_hard_pseudo_label(self, soft_pseudo_label):
        max_p, hard_pseudo_label = paddle.max(soft_pseudo_label, dim=-1) # hard_pseudo_label means real label rather than sofmax
    ...
    def train_semiFed_model_labeled_client_PC(self, round, epoch, dataloader,  device):
        ...
        logging.info('\n=> Labeled Client SemiFed Training Epoch #%d, LR=%.4f' % (self.trainer_update_epoch, self.optimizer.get_lr()))
        for batch_idx, (x_w , x_s, y) in enumerate(dataloader):
            real_bs = x_w.shape[0]

            x_w, x_s, y = x_w.to(device), x_s.to(device), y.to(device)

            # y = paddle.cat((y, y), dim=0)
            ...
            unsup_sim_f = unsup_sim_f / paddle.norm(unsup_sim_f, axis=1, keepdim=True)
            sup_sim_f = sup_sim_f / paddle.norm(sup_sim_f, axis=1, keepdim=True)

            unsup_gram_matrix = paddle.mm(unsup_sim_f, unsup_sim_f.t())
            sup_gram_matrix = paddle.mm(sup_sim_f, sup_sim_f.t())
            gram_loss = paddle.sum(paddle.pow((unsup_gram_matrix - sup_gram_matrix), 2) / (real_bs ^ 2))
            sim_loss = gram_loss
    ...
    def train_semiFed_model_unlabeled_client_PC(self, round, epoch, dataloader,  device):
        ...
        criterion1 = nn.loss.CrossEntropyLoss()
        ...
        p_model = (paddle.ones(self.args.num_classes) / self.args.num_classes).to(device)
        label_hist = (paddle.ones(self.args.num_classes) / self.args.num_classes).to(device)
        time_p = p_model.mean()
        logging.info('\n=> UnLabeled Client SemiFed Training Epoch #%d, LR=%.4f' % (self.trainer_update_epoch, self.optimizer.get_lr()))

        for batch_idx, (x_w , x_s) in enumerate(dataloader):
            real_bs = x_w.shape[0]

            x_w, x_s = paddle.to_tensor(x_w, place=device), paddle.to_tensor(x_s,place=device)
    ...
    def train_semiFed_model_PD(self, round, epoch, global_params, 
        ...
        for batch_idx, (x_ulb_w, x_ulb_s) in enumerate(unlabled_dataloader):
            ...
            self.optimizer.zero_grad()
            inputs_w = paddle.concat([x_lb_w, x_ulb_w])
            inputs_s = paddle.concat([x_lb_s, x_ulb_s])
            ...
            unsup_sim_f = unsup_sim_f / paddle.norm(unsup_sim_f, dim=1, keepdim=True)
            sup_sim_f = sup_sim_f / paddle.norm(sup_sim_f, dim=1, keepdim=True)
            unsup_gram_matrix = paddle.mm(unsup_sim_f, unsup_sim_f.t())
            sup_gram_matrix = paddle.mm(sup_sim_f, sup_sim_f.t())
            gram_loss = paddle.sum(paddle.pow((unsup_gram_matrix - sup_gram_matrix), 2) / (inputs_bs ^ 2))
            sim_loss = gram_loss
            ...
            if self.args.fedprox:
                ...
                for name, param in self.model.named_parameters():
                    fed_prox_reg += ((self.args.fedprox_mu ) * \
                        paddle.norm((param - global_params[name].data.to(device)))**2)
                loss += fed_prox_reg
            # ========================FedProx=====================#
            loss.backward()
            self.optimizer.step()

            prec1, prec5, correct, pred, _ = accuracy(sup_logits[:lb_data_bs].data, y.data)
...
def consistency_loss_fixmatch(logits_w, logits_s, T=1.0, p_cutoff=0.0, use_hard_labels=True):
    ...
    pseudo_label = paddle.softmax(logits_w, dim=-1)
    max_probs, max_idx = paddle.max(pseudo_label, dim=-1)
    ...
    if use_hard_labels:
        ...
        pseudo_label = paddle.softmax(logits_w / T, dim=-1)
...
def ce_loss(logits, targets, use_hard_labels=True, reduction='none'):
    ...
    if use_hard_labels:
        ...
        log_pred = F.log_softmax(logits, dim=-1)
        nll_loss = paddle.sum(-targets * log_pred, dim=1)
        return nll_loss
...
def symmetric_mse_loss(input1, input2):
    ...
    return paddle.sum((input1 - input2)**2) / num_classes
...
def network_add(diff_w_ema_1, diff_w_ema_2):
    ...
    for key in diff_w_ema_1.keys():
        diff_w_ema[key] = paddle.sqrt(diff_w_ema_1[key] + diff_w_ema_1[key]) / paddle.sqrt(diff_w_ema_2[key] + diff_w_ema_2[key])
